...
from app.utils import login_required, admin_required, manager_or_admin_required, log_audit, get_db
...
def attendance_dashboard():
    ...
    try:
        ...
        logger.error("Attendance dashboard error: %s", e)
...
def process_attendance():
    ...
    try:
            ...
            with conn.cursor() as cursor:
                ...
                if action == 'check_in':
                    ...
                    if not record:
                        ...
                        cursor.execute("""
                            UPDATE attendance SET check_out = %s WHERE emp_id = %s AND date = %s
        ...
        logger.error("Process attendance error: %s", e)
        flash('An unexpected error occurred.', 'danger')
...
def attendance_history():
    ...
    try:
        ...
        logger.error("Attendance history error: %s", e)
        flash('An unexpected error occurred.', 'danger')
...
def payroll_dashboard():
    ...
    try:
        ...
        logger.error("Payroll dashboard error: %s", e)
        flash('An unexpected error occurred.', 'danger')
...
def generate_payroll():
    ...
    month = safe_int(request.form.get('month'), datetime.now().month)
    ...
    try:
        ...
        logger.error("Generate payroll error: %s", e)
        flash('An unexpected error occurred.', 'danger')
